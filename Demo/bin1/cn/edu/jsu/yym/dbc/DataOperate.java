...
public class DataOperate {
    ...
    public static double getNum1(double d,double end) {//随机返回返回指定范围间的整数
    	//Math.random()随机返回0.0至10之间的数
    ...
    public static void addCJ() {//增加成绩
    	...
    	String sql="insert into student1(xh,xm,cj,no1,no2,no3,no4,no5) values(?,?,?,?,?,?,?,?)";//使用占位符定义插入语句
    	try(Connection conn=dbcs.getConnection();//获取数据库接
    		PreparedStatement slh=conn.prepareStatement(sql);){//实例化PreparedStatement
    		...
    		for(int i=1;i<=10000;) {		
    			...
    			if(!alist.contains(xh)) {//判断学号是否唯一
    				...
    				Double no1=getNum1(0, 9.00) ;//随机获取成绩
    	        	Double no2=getNum1(0, 9.00);//随机获取成绩
    	        	Double no3=getNum1(0, 9.00);//随机获取成绩
    	        	Double no4=getNum1(0, 9.00);//随机获取成绩
    	        	Double no5=getNum1(0, 9.00);//随机获取成绩
    	        	
    	        	a=String.valueOf(no1);
    	        	b=String.valueOf(no2);
    	        	c=String.valueOf(no3);
    	        	d=String.valueOf(no4);
    	        	e=String.valueOf(no5);//把各double类型的分数转换成string类型，
    	        	
    	        	double a1=Double.valueOf(a);
    				double a2=Double.valueOf(b);
    				double a3=Double.valueOf(c);
    				double a4=Double.valueOf(d);
    				double a5=Double.valueOf(e);//把string类型的分数转换成double类型
    				cj=DataOperate.averg(a1, a2, a3, a4, a5);
    				//获取随机平均成绩
    				slh.setString(1, xh);//定义第1个占位符的内容
    	    		slh.setString(2, xm);//定义第2个占位符的内容
    	    		slh.setDouble(3, cj);//定义第3个占位符的内容   	    		
					slh.setDouble(4, no1);
    	    		slh.setDouble(5, no2);
    	    		slh.setDouble(6, no3);
    	    		slh.setDouble(7, no4);
    	    		slh.setDouble(8, no5);
    	    		slh.addBatch();//加入批处理等待执行
    	    		System.out.print(i+"\n");
    				i++;//学号唯一，循环继续往下执行
    			}
    		}
